...
class CatchesVerboseSerializer(serializers.ModelSerializer):
    ...
    measurement_type_id = serializers.IntegerField(source='sp.measurement_type_id', read_only=True)

    class Meta:
        model = Catch
        fields = ['catch_id', 'category', 'weight', 'not_measured_individuals', 'haul', 'haul_id', 'group', 'sp_id',
                  'sp_code', 'sp_name', 'unit', 'increment', 'sampled_weight', 'measurement_type_id', ]
