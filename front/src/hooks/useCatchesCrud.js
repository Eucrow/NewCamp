...
export const useCatchesCrud = haul_id => {
  ...
  const apiEditRemoveCatch = buildApiUrl(
    API_CONFIG.ENDPOINTS.EDIT_REMOVE_CATCH
  );
  ...
  const updateCatch = async idx => {
    try {
      ...
      const requestData = {
        ...
        sampled_weight:
          updatedCatch.sampled_weight === "0"
            ? null
            : updatedCatch.sampled_weight,
      ...
      setCatches(prevCatches =>
        prevCatches.map(c => (c.catch_id === idx ? data : c))
      );
  ...
  useEffect(() => {
    const fetchCatches = async () => {
      ...
      const apiCatches = buildApiUrl(
        API_CONFIG.ENDPOINTS.CATCHES_BY_HAUL(haul_id)
      );
      ...
      try {
        ...
        if (!data.ok) {
          throw new Error(
            "Something went wrong! " + data.status + " " + data.statusText
          );
        }
